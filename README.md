...
- 支持分组管理、批量设置分组、批量删除、批量复制账号/密码/账号----密码
- 单账号“同步查看”会打开独立邮件弹窗，按邮件列表 + 详情布局展示
...
- 自动化测试覆盖导入解析、导入导出接口、分组与批量操作接口、同步成功/失败链路、批量节流重试、OAuth token 刷新逻辑
...
- 分组有独立颜色配置；删除自定义分组时，分组下账号会自动回退到“默认分组”。
- 批量复制支持 3 种格式：`账号`、`密码`、`账号----密码`。
